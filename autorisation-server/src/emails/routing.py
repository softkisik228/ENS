...
# async def add_emails(new_email: EmailAdd, session: AsyncSession = Depends(get_async_session)):
#     query = select(email).where(email.c.id == new_email.model_dump()["id"])
#     result = await session.execute(query)
#     em = result.fetchone()
#     if em == None:
#         stmnt = insert(email).values(new_email)
#         await session.execute(stmnt)
#         await session.commit()
#     else:
#         combined = {**em.emails, **new_email.model_dump()['emails']}
#         stmt = (
#         update(email)
#         .where(email.c.id == new_email.model_dump()["id"])
